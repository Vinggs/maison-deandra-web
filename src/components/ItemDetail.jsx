...
// 🔥 PENTING: Tambahkan isAppLoading di prop
const ItemDetail = ({
  item,
  allOutfits,
  onBack,
  ...
  // 🆕 STATE UNTUK LOADING GAMBAR AGAR TIDAK GLITCH
  ...
  useEffect(() => {
    ...
    // Reset state ke false setiap kali item berubah
    ...
    if (item && item.image) {
      ...
      img.onerror = () => setMainImageLoaded(true); // Tetap lanjut jika error
    } else {
  ...
  return (
    <>
      {/* 🔥 PERBAIKAN: Loading ini hanya muncul jika GAMBAR BELUM SIAP DAN APP SUDAH SELESAI LOADING */}
      {!mainImageLoaded && !isAppLoading && (
        <div className="fixed inset-0 z-[60] bg-[#F5F4F1] flex flex-col items-center justify-center">
          <p className="font-display text-4xl text-stone-900 mb-4 italic animate-pulse">
            Loading
          </p>
      ...
      {/* KONTEN UTAMA: Disembunyikan (opacity-0) sampai gambar siap */}
      <div
        className={`max-w-7xl mx-auto px-6 md:px-10 py-10 transition-opacity duration-1000 ${mainImageLoaded ? "opacity-100" : "opacity-0 overflow-hidden h-screen"}`}
      >
        <button
          onClick={onBack}
